    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_match (idMatch, dateMatch) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            str_sql_update_nom_film = """UPDATE t_match SET dateMatch = %(nom_film_update)s,
                                                            WHERE idMatch = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_match WHERE idMatch = %(value_id_film)s"
            ...
            form_update_film.nom_film_update_wtf.data = data_film["dateMatch"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_choix WHERE fk_match = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_match WHERE idMatch = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT idMatch, dateMatch, pChoix, SUM(pChoix) AS somme FROM t_match 
            INNER JOIN t_choix ON t_choix.fk_match = t_match.idMatch
            WHERE idMatch = %(value_id_film)s
            GROUP BY dateMatch
            HAVING pChoix = 1
            """
