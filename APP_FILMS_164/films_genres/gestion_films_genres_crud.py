    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT * FROM t_choix 
                    INNER JOIN t_match ON t_match.idMatch = t_choix.fk_match
                    INNER JOIN t_personne ON t_personne.idPersonne = t_choix.fk_personne
                    """
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """WHERE pChoix = %(value_id_film_selected)s
                                                            ORDER BY dateMatch"""

                    # Rework : requête SQL affichant idMatch, Nom / Prenom de la personne, FK associée, Choix (0 => pas sûr, 1 => Ok, 2 => Pas ok), idChoix + date Match
                    # La modification de l'URL fera intéragir 'pChoix' avec un WHERE. Offrant donc les choix des personnes, ordré par date de match.
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "tPersonne" est vide. !""", "warning")
...
# Zone supprimée, non utilisée.
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT * FROM t_choix 
                    INNER JOIN t_match ON t_match.idMatch = t_choix.fk_match
                    INNER JOIN t_personne ON t_personne.idPersonne = t_choix.fk_personne
                    """
            ...
            lst_data_film_selected = [item['dateMatch'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['dateMatch'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['dateMatch'] for item in data_genres_films_attribues]
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT * FROM t_choix 
                    INNER JOIN t_match ON t_match.idMatch = t_choix.fk_match
                    INNER JOIN t_personne ON t_personne.idPersonne = t_choix.fk_personne"""

        strsql_genres_films_non_attribues = """SELECT * FROM t_choix 
                    INNER JOIN t_match ON t_match.idMatch = t_choix.fk_match
                    INNER JOIN t_personne ON t_personne.idPersonne = t_choix.fk_personne"""

        strsql_genres_films_attribues = """SELECT * FROM t_choix 
                    INNER JOIN t_match ON t_match.idMatch = t_choix.fk_match
                    INNER JOIN t_personne ON t_personne.idPersonne = t_choix.fk_personne"""
